...
let DELETE_ICON = 'https://img.drawai.cn/icon/canvas_icon2.png'; // 删除按钮
// 删除按钮
let DRAG_ICON = 'https://img.drawai.cn/icon/canvas_icon1.png'; // 缩放按钮
// 缩放按钮
...
dragGraph.prototype = {
  ...
  paint() {
    ...
    if (this.type === 'text') {
      ...
      // console.log('>>>>>>>>>>>>>>drawImage', this.fileUrl, this.x, this.y, this.w, this.h);
    ...
    if (this.selected) {
      ...
      if (this.type === 'text') {
        ...
        this.showDelete && this.ctx.drawImage(DELETE_ICON, this.x - 10, this.y - 10, 20, 20);
        this.ctx.drawImage(DRAG_ICON, this.x + textWidth - 15, this.y + textHeight - 15, 20, 20);
      } else {
        this.ctx.strokeRect(this.x, this.y, this.w, this.h);
        this.showDelete && this.ctx.drawImage(DELETE_ICON, this.x - 10, this.y - 10, 20, 20);
        this.ctx.drawImage(DRAG_ICON, this.x + this.w - 15, this.y + this.h - 15, 20, 20);
      }
    }
  ...
  isInGraph(x, y) {
    ...
    if (x - transformScaleX >= 0 && y - transformScaleY >= 0 && transformScaleX + scaleW - x >= 0 && transformScaleY + scaleH - y >= 0) {
      // 操作属于缩放旋转区域
      return 'transform';
    } else if (this.showDelete && (x - transformDelX >= 0 && y - transformDelY >= 0 && transformDelX + delW - x >= 0 && transformDelY + delH - y >= 0)) {
...
export default {
  ...
  paint() {
    ...
    if (this.selected) {
      ...
      if (this.type === 'text') {
        this.ctx.strokeRect(this.x, this.y, textWidth, textHeight);
        this.showDelete && this.ctx.drawImage(DELETE_ICON, this.x - 15, this.y - 15, 30, 30);
  ...
  methods: {
    ...
    onGraphChange(n, o) {
      ...
      // console.error('--------------onGraphChange', n);
      this.putImgWidth = n.canvasWidth
      this.putImgHeigt = n.canvasHeight
      this.drawArr = [new dragGraph(Object.assign({
        x: n.x,
        y: n.y,
    ...
    draw() {
      if (this.bgImage !== '') {
        this.ctx.drawImage(this.bgImage, 0, 0, 360, 360);
      }
    ...
    exportFun() {
      return new Promise((resolve, reject) => {
        ...
        let canvasData = {
          ...
          destWidth: this.putImgWidth,
          destHeight: this.putImgHeigt,
        }
        // #ifdef MP
        ...
        // #endif
        console.log('输入画布内容...canvasData', canvasData);
        this.draw().then(() => {
