	...
	export default {
		...
		methods: {
			...
			allStatusList(status){
				let that =this;
				that.leftList.map(item=>{
					item.statusBox=status;
				})
				that.rightList.map(item=>{
					item.statusBox=status;
				})
				that.leftList=[...that.leftList];
				that.rightList=[...that.rightList];
				// 个数统计
                this.$emit("clickBox",that.numberSize());
			},
			...
			albumDelete(){
				let that =this;
				let albumList =that.leftList.concat(that.rightList);
				let arr=[];
			...
			startRender(){
				...
				if(!this.$props.list || this.$props.list.length < 1){
					console.log('瀑布流插件提示：当前数据为空，不会触发列表渲染');
					return;
				}
