  ...
  >
    ...
    <view class="flex-col flex-auto group_3">
      ...
      <view class="user-power">
        <view :style="{ 'color': cueTheme == 'day-theme' ? '#000' : '#1AFFE8' }">关键词公开范围：
          <block v-if="draw.isVisible == 0">所有人不可画同款</block>
          <block v-else-if="draw.isVisible == 1">所有人可画同款</block>
          <block v-else-if="draw.isVisible == -1">支付{{ draw.isVisibles }}灵力画同款</block>
          <block v-else-if="draw.isVisible == -2">支付{{ draw.isVisibles }}灵力画同款</block>
          <block v-else-if="draw.isVisible == -3">支付{{ draw.isVisibles }}灵力画同款</block>
          <block v-else>支付{{ draw.isVisibles }}灵力画同款</block>
        </view>
        <view
          class="btn"
          v-if="type != 3 && draw.userId == userInfo.id"
      ...
      <view class="flex-col group_4 space-y-22">
        ...
        <view class="items-baseline space-x-2">
          <view
            class="font_1"
            @click="copyKeyword(draw.prompt, '关键词')"
            :style="{ 'color': cueTheme == 'day-theme' ? '#000' : '#8A8A8A' }"
          >关键词：(点击复制)</view>
          <view
            class="font_2"
            @click="copyKeyword(draw.prompt, '关键词')"
            :style="{ 'color': cueTheme == 'day-theme' ? '#000' : '#fff' }"
          >{{ draw.prompt ? draw.prompt : '未公开' }}</view>
        </view>
        <view class="items-baseline space-x-2">
          <view
            class="font_1"
          ...
          <view
            class="font_2"
            :style="{ 'color': cueTheme == 'day-theme' ? '#000' : '#fff' }"
            v-if="draw.userId == userInfo.id"
          >
            {{ goods.modelInfo.modelName }}</view>
          <view
            class="font_2"
            :style="{ 'color': cueTheme == 'day-theme' ? '#000' : '#fff' }"
    ...
    moveToGallery() {
      ...
      moveToGallery(data).then(res => {
        if (res.code == 200) {
          ...
          that.galleryDetail(that.albumId, '');
        } else {
          this.moveModalShow = false;
        }
      })
