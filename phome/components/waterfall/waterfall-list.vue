		...
		<view class="waterfall-box h-flex-x h-flex-2" v-if="showList">
			<view>
				<view v-for="(item,index) in leftList" :key="item._render_id" class="list-item" 
					...
					<helang-waterfall-item ref="child"
						:params="item" 
						tag="left"
						:index="index"
						:statusBox="statusBox"
...
<script>
import {deleteBatch,detail} from '../../../static/api/analysis.js'
	...
	export default {
		...
		methods: {
			...
			albumDelete(){
				uni.showModal({
					...
					if (res.confirm) {
						...
						if(arr.length==0){
							uni.showToast({ title: "请选择需要删除的图片",icon:"none",duration: 3000 })
							return
						}
						deleteBatch({ids:arr}).then(res => {
							if(res.code ==200){
			...
			// 渲染列表，这里实现瀑布流的左右分栏
			renderList() {
				// console.log(this.awaitRenderList.length,'this.awaitRenderList.length')
				// 待渲染长度为 0 时表示已渲染完成
				...
				// console.log(item,'是数据呀')
				if(item.status==2 ){
					let that =this;
					 setTimeout(function(){
						that.getDetails(item.id)
					 },5000)
				}
				...
				detail({id:id}).then(res => {
					if(res.code ==200){
						...
						if(res.result.status==2){
							...
							that.leftList[0].status=res.result.status;
						}
					}
				})
			},
			...
			startRender(){
				...
				// console.log(this.awaitRenderList ,'this.awaitRenderList ')
				this.renderList();
			}
		}
	}
