	...
	<view class="u-wrap" :style="{
	 ...
	 :class="'u-lazy-item-' + elIndex">
		<view :class="'u-lazy-item-' + elIndex">
			<image :webp="true" :style="{borderRadius: borderRadius + 'rpx', height: imgHeight}" v-if="!isError" class="u-lazy-item"
			 :src="isShow ? image : loadingImg" :mode="imgMode" @load="imgLoaded" @error="loadError" @tap="clickImg"></image>
			<image :webp="true" :style="{borderRadius: borderRadius + 'rpx', height: imgHeight}" class="u-lazy-item error" v-else :src="errorImg"
...
<script>
	...
	export default {
		...
		computed: {
			...
			imgHeight() {
				return uni.$u.addUnit(this.height);
			}
		},
		created() {
