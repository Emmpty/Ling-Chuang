<template>
	<view class="waterfall-item-container" :style="{'background':cueTheme=='day-theme'?'#f5f5f5':'#262628'}">
		<view class="waterfall-item" @tap="onTap">
			...
			<view class="defeated" v-if="params.status == -3" style="color:#FF6850;" @click="albumDetails(params.id)">制作超时</view>
			<view class="defeated" v-if="params.status == -5" style="color:#FF6850;" @click="albumDetails(params.id)">参数有误</view>
			<view class="defeated" v-if="params.status == -7" style="color:#FF6850;" @click="albumDetails(params.id)">链接异常</view>
			<view class="defeated" v-if="params.status == -8" style="color:#FF6850;" @click="albumDetails(params.id)">审核不通过</view>
			<view class="defeated" v-if="params.status >0" style="color:#1AFFE8;" @click="albumDetails(params.id)">正在制作中</view>
		</view>
	</view>
</template>
