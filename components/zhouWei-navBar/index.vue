        ...
        <view class="header_left_box">
          <slot name="left">
            <view
              ...
              :style="useBgColor ? `background-color:${bgColor};` : ''"
            ...
            >
              ...
              >
                <image
                  class="header_icon"
                  v-if="isWhite"
                  src="/static/img/tabbar/nav_arr.png"
                  mode="aspectFit"
                ></image>
                <image
                  class="header_icon"
                  v-else
                  src="/static/img/tabbar/nav_arr1.png"
                  mode="aspectFit"
                ></image>
              </view>
              <text
              ...
              >
                <image
                  class="header_icon"
                  v-if="isWhite"
                  src="/static/img/tabbar/nav_home.png"
                  mode="aspectFit"
                ></image>
                <image
                  class="header_icon"
                  v-else
                  src="/static/img/tabbar/nav_home1.png"
                  mode="aspectFit"
                ></image>
              </view>
            </view>
...
const mainPagePath = ['/pages/tabBar/gallery'];
//返回首页的地址
const homePath = '/pages/tabBar/gallery';
//白色表达值
...
export default {
  ...
  computed: {
    ...
    // 右上角是否有两个按钮
    isTwoBtn() {
      return (this.backState == 1000 || this.backState == 3000) && this.homeState == 1000;
    }
  },
  watch: {
    title(val) {
  ...
  created() {
    ...
    this.navFontColor = this.fontColor || this.cueTheme == 'day-theme' ? '#000000' : '#ffffff';
  ...
  methods: {
    ...
    pageScroll(e) {
      if (this.type == 'transparentFixed') {
        if (e.scrollTop && e.scrollTop > 0) {
          if (e.scrollTop > this.transparentScrollTop) {
            this.transparentValue = 1;
          } else {
            this.transparentValue = e.scrollTop / this.transparentScrollTop;
          }
        } else {
    ...
    getNavBgColor(val) {
      if (val === '#FFFFFF') {
        ...
        let bgColor = this.cueTheme == 'day-theme' ? '#ffffff' : '#141416'
        this.getNavBgColor(bgColor);
        return
      } else if (val != '#ffffff' && val != '#141416') {
    ...
    settingColor() {
      ...
      setTimeout(() => {
        ...
        uni.setBackgroundTextStyle({
          textStyle: this.cueTheme == 'day-theme' ? 'dark' : 'light'
        })
      }, 150);
    }
  }
...
.header_left_line {
  height: 30rpx;
  width: 2rpx;
  background-color: var(--border-color-nav);
}
...
.header_btnMongol {
  ...
  border-style: solid;
  border-width: 2rpx;
  border-color: var(--border-color-nav);
