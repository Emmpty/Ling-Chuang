...
export default {
  ...
  computed: {
     ...
     */
    defCanvasStyle() {
      return `width:${this.width}px;height:${this.height}px;background:#000`;
    }
  },
  ...
  methods: {
    ...
    getmycanvasId(bgImage, url) {
      let CanvasContext = uni.createCanvasContext("mycanvasId", this);
      CanvasContext.setFillStyle('#000000');  // 背景颜色
      CanvasContext.fillRect(0, 0, this.width, this.height)
      CanvasContext.drawImage(bgImage, 0, 0, this.width, this.height);
      CanvasContext.draw();
      setTimeout(_ => {
        uni.canvasToTempFilePath({
          ...
          // fileType:'jpg',
          success: res => {
            this.boxImgas = res.tempFilePath
            return url(res.tempFilePath)
          },
          fail: err => {
      ...
      return new Promise((resolve, reject) => {
        setTimeout(_ => {
          uni.canvasToTempFilePath({
            ...
            // fileType:'jpg',
            success: res => {
              this.getmycanvasId(res.tempFilePath, function (url) {
                resolve(url);
              })
            },
            fail: err => {
